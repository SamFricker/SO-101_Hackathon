...
class EnvironmentState:
    """External environment readings (e.g. humidity from IoT sensor)."""

    def __init__(self) -> None:
        """Initialize EnvironmentState with default values."""
        self._lock = threading.Lock()
...
class DataManager:
    ...
    def __init__(self) -> None:
        ...
        self._overhead_camera_state = CameraState()
    ...
    def set_rgb_image(self, image: np.ndarray) -> None:
        """Set wrist/workspace RGB image (thread-safe)."""
        with self._camera_state._lock:
    ...
    def get_overhead_rgb_image(self) -> np.ndarray | None:
        """Get overhead RGB image (thread-safe)."""
        with self._overhead_camera_state._lock:
            return (
                self._overhead_camera_state.rgb_image.copy()
                if self._overhead_camera_state.rgb_image is not None
                else None
            )

    def set_overhead_rgb_image(self, image: np.ndarray) -> None:
        """Set overhead RGB image (thread-safe)."""
        with self._overhead_camera_state._lock:
            self._overhead_camera_state.rgb_image = image.copy()
        if self._on_change_callback:
            self._on_change_callback(
                "log_rgb_overhead",
                self._overhead_camera_state.rgb_image.copy(),
    ...
    def get_humidity_percent(self) -> float | None:
        """Get latest humidity reading in percent (thread-safe)."""
        with self._environment_state._lock:
            return self._environment_state.humidity_percent

    def set_humidity_percent(self, value: float) -> None:
        """Set humidity reading in percent (thread-safe)."""
        with self._environment_state._lock:
            self._environment_state.humidity_percent = float(value)
        if self._on_change_callback:
            self._on_change_callback(
                "log_humidity",
                self._environment_state.humidity_percent,
                time.time(),
            )
