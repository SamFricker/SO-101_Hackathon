"""Vendored Siglip2 vision model from HuggingFace/NVIDIA."""

# ruff: noqa: E501
# noqa: D
# cspell:ignore varlen unpad softcap seqlens naflex selu idxs bchw patchified
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
def _flash_attention_forward(
    ...
    **kwargs: Any,
    ...
    """Call the forward method of Flash Attention.

    If the input hidden states contain at least one padding token
    ...
    Args:
        ...
        position_ids (`torch.Tensor`, *optional*):
            Position IDs for the query.
        softmax_scale (`float`, *optional*):
            The scaling of QK^T before applying softmax. Default to 1 / sqrt(head_dim).
        sliding_window (`int`, *optional*):
            Sliding window size for local attention.
        use_top_left_mask (`bool`, *optional*):
            Whether to use top-left causal mask.
        softcap (`float`, *optional*):
            Soft cap value for attention logits.
        deterministic (`bool`, *optional*):
            Whether to use deterministic attention.
        cu_seq_lens_q (`torch.LongTensor`, *optional*):
            Cumulative sequence lengths for queries.
...
def flash_attention_forward_for_packing(
    ...
    sliding_window: int | None = None,
    softcap: float | None = None,
    seq_len_list: list[int] | None = None,
    **kwargs: Any,
...
class Siglip2VisionConfig(PretrainedConfig):
    r"""Configuration class for Siglip2VisionModel.
    ...
    def __init__(
        ...
        num_channels: int = 3,
        num_patches: int = 256,
        patch_size: int = 14,  # manually modified
        hidden_act: str = "gelu_pytorch_tanh",
        layer_norm_eps: float = 1e-6,
        attention_dropout: float = 0.0,
        window_size: int = 14,
        full_attention_indexes: list[int] | None = None,
        use_rope: bool = True,
        use_windows_attn: bool = True,
        **kwargs: Any,
    ) -> None:
...
def convert_image_to_patches(image: "torch.Tensor", patch_size: int) -> "torch.Tensor":
    """Convert 3D tensor image into 2D tensor of patches.

    Converts shape (num_channels, image_height, image_width) into
...
def convert_images_to_patches(image: "torch.Tensor", patch_size: int) -> "torch.Tensor":
    """Convert 4D tensor image into 2D tensor of patches.

    Converts shape (batch_size, num_channels, image_height, image_width) into
...
class Siglip2VisionEmbeddings(nn.Module):
    ...
    def split_patch_embeddings_to_windows_with_meta(
        self, patch_embeds: torch.Tensor, batch_hw: Any, window_size: int
    ) -> tuple[torch.Tensor, list[dict[str, Any]], torch.Tensor]:
        """Split patch embeddings into windows with metadata.
        ...
        Returns:
            Tuple of (windows_tensor, win_meta_list, reverse_mapping).
        """
        # 1. Compute start offset of each image in the flattened tensor
        batch_hw = batch_hw.tolist()
        counts = [H * W for (H, W) in batch_hw]
        starts = [0] + list(accumulate(counts))[:-1]

        # 2. Group by (H, W); process all images of the same size together
        ...
        all_meta = []
        # print(size2info)
        # 3. For each size group, batch unfold + pad
        for (H, W), info in size2info.items():
            ...
            # 3.2 Pad (bottom, right) so H and W are divisible by window_size
            ...
            for b, img_idx in enumerate(img_idxs):
                for win_id in range(n_windows):
                    ...
                    all_meta.append({
                        ...
                        "patch_hw": (H, W),
                        "win_xy": (h0, w0),
                        "win_hw": (h1 - h0, w1 - w0),  # valid region size
                    })
        ...
        for meta, win in zip(win_meta_list, all_windows):
            ...
            # Keep only patch tokens that lie inside the original image
        ...
        # 1. Recompute each image's start offset in the original flattened tensor
        counts = [H * W for H, W in batch_hw]
        starts = [0] + list(accumulate(counts))[:-1]
        ...
        offset = 0  # cursor along the all_tokens sequence
        ...
        for meta in win_meta_list:
            ...
            # Map every patch token in this window that lies in the original image
            for u in range(h_eff):
                for v in range(w_eff):
                    # Original flat index
                    orig_idx = base + (h0 + u) * W + (w0) + v
                    # Index in all_tokens: row-major within this window segment
                    p = u * w_eff + v
                    mapping[orig_idx] = offset + p

            # After each window, advance offset by its number of valid tokens
            offset += h_eff * w_eff
...
class Rope2DPosEmb(nn.Module):
    ...
    def get_freqs_cis(
        self, win_meta_list: list[dict[str, Any]], device: torch.device
    ) -> torch.Tensor:
        """Get frequency cis tensors for the given window meta list.
...
def _apply_rope_input_validation(x: torch.Tensor, freqs_cis: torch.Tensor) -> None:
...
class Siglip2Attention(nn.Module):
    ...
    def forward(
        ...
        win_meta_list: list[dict] | None = None,
        windows_attn: bool | None = False,
        ...
        assert win_meta_list is not None
        if windows_attn and self.use_windows_attn:
            ...
            mapper: dict[int, int] = defaultdict(int)
            for win_meta in win_meta_list:
...
class Siglip2EncoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            rope_freqs_cis (`torch.Tensor`, *optional*):
                Precomputed 2D rotary position embedding frequencies.
            win_meta_list (`list[dict]`, *optional*):
                Window metadata list for windowed attention.
            windows_attn (`bool`, *optional*, defaults to `False`):
                Whether to use windowed attention.
        """
...
class Siglip2Encoder(nn.Module):
    ...
    Each layer is a [`Siglip2EncoderLayer`].
    ...
    def forward(
        ...
        win_meta_list: list[dict[str, Any]] | None = None,
        spatial_shapes: torch.Tensor | None = None,
    ) -> BaseModelOutput:
        r"""Forward pass for the encoder.
        ...
        Args:
            ...
            win_meta_list (`list[dict]`, *optional*):
                Window metadata list for windowed attention.
            spatial_shapes (`torch.Tensor`, *optional*):
                Spatial shapes of images for positional embeddings.
        """
        output_attentions = (
            output_attentions
...
def build_idx_map(
    ...
    counts = [h * w for (h, w) in spatial_shapes_list]
    starts = [0] + list(accumulate(counts))[:-1]
    total_patches = sum(counts)

    mapping: list[int | None] = [None] * total_patches
    offset = 0

    for win_meta in win_meta_list:
        img_idx = int(win_meta["img_idx"])
        h_img, w_img = spatial_shapes_list[img_idx]
        ...
        for u in range(h_eff):
            for v in range(w_eff):
                orig_idx = base + (h0 + u) * w_img + w0 + v
                p = u * w_eff + v
                mapping[orig_idx] = offset + p

        offset += h_eff * w_eff

    missing = [i for i, v in enumerate(mapping) if v is None]
    if missing:
        raise ValueError(
            "Could not build index map for all patches. "
            f"Missing {len(missing)} entries; first few: {missing[:10]}"
        )
...
class Siglip2VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        r"""Return model outputs.

        Returns:
            BaseModelOutputWithPooling: Model outputs.
...
def _trunc_normal_(
    ...
    # [2l-1, 2u-1].
    tensor.uniform_(2 * lower_cdf - 1, 2 * u - 1)
...
class Siglip2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Module) -> None:
        ...
        if isinstance(module, Siglip2VisionEmbeddings):
            ...
            # Siglip2Model (full model, not used here — only vision model is needed)
            logit_scale_init = torch.log(torch.tensor(1.0))
            module.logit_scale.data.fill_(logit_scale_init)
            module.logit_bias.data.zero_()
        elif hasattr(module, "classifier") and isinstance(module.classifier, nn.Linear):
            # Siglip2ForImageClassification (not used here)
            nn.init.normal_(
