...
# --------------------------------------------------------
"""Eagle3 Vision-Language model implementation (vendored from NVIDIA)."""

# ruff: noqa: E501
# noqa: D
# cspell:ignore neftune unshuffle idxs
...
try:
    from transformers import InternVisionModel  # type: ignore[attr-defined]
except Exception:  # pragma: no cover
    InternVisionModel = None  # type: ignore[assignment]

try:
    from transformers import RADIOModel  # type: ignore[attr-defined]
except Exception:  # pragma: no cover
    RADIOModel = None  # type: ignore[assignment]
...
class Eagle3_VLPreTrainedModel(PreTrainedModel):
    """Pre-trained model base class for Eagle3 Vision-Language models."""
...
class Eagle3_VLForConditionalGeneration(Eagle3_VLPreTrainedModel, GenerationMixin):
    """Eagle3 Vision-Language model for conditional generation."""
    ...
    def __init__(
        ...
        """Initialize Eagle3_VLForConditionalGeneration."""
        ...
        if vision_model is not None:
            ...
            if config.vision_config.model_type == "intern_vit_6b":
                if InternVisionModel is None:
                    raise ImportError(
                        "InternVisionModel is not available in this environment. "
                        "Install the required dependency or use a supported `vision_config.model_type`."
                    )
                ...
                if RADIOModel is None:
                    raise ImportError(
                        "RADIOModel is not available in this environment. "
                        "Install the required dependency or use a supported `vision_config.model_type`."
                    )
    ...
    def check_forward_kwargs(self) -> None:
        """Verify that forward method does not use **kwargs."""
    ...
    def wrap_backbone_lora(
        self, r: int = 128, lora_alpha: int = 256, lora_dropout: float = 0.05
    ) -> None:
        """Wrap the vision backbone with LoRA adapters."""
    ...
    def forward(
        ...
        """Perform forward pass for conditional generation."""
        return_dict = (
    ...
    def pixel_shuffle_back(
        self, vit_embeds: torch.Tensor, spatial_shapes: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Reverse pixel shuffle to downsample vision embeddings."""
    ...
    def mask_valid_tokens(
        self,
        vit_embeds: torch.Tensor,
        spatial_shapes: torch.Tensor,
        image_flags: torch.Tensor,
    ) -> torch.Tensor:
        """Mask and return only valid tokens based on image flags."""
    ...
    def extract_feature(
        self, pixel_values: list[torch.FloatTensor], image_flags: Any = None
    ) -> torch.Tensor:
    ...
    def generate(
        ...
        **generate_kwargs: Any,
        ...
        """Generate text tokens from visual and text inputs."""
    ...
    def set_input_embeddings(self, value: nn.Module) -> None:
        """Set the input embeddings on the language model."""
        self.language_model.set_input_embeddings(value)

    # Copied from transformers.models.llava_next.modeling_llava_next.LlavaNextForConditionalGeneration.get_output_embeddings
    def get_output_embeddings(self) -> nn.Module:
        """Get the output embeddings from the language model."""
        return self.language_model.get_output_embeddings()

    # Copied from transformers.models.llava_next.modeling_llava_next.LlavaNextForConditionalGeneration.set_output_embeddings
    def set_output_embeddings(self, new_embeddings: nn.Module) -> None:
        """Set the output embeddings on the language model."""
        self.language_model.set_output_embeddings(new_embeddings)

    # Copied from transformers.models.llava_next.modeling_llava_next.LlavaNextForConditionalGeneration.set_decoder
    def set_decoder(self, decoder: nn.Module) -> None:
        """Set the decoder on the language model."""
        self.language_model.set_decoder(decoder)

    # Copied from transformers.models.llava_next.modeling_llava_next.LlavaNextForConditionalGeneration.get_decoder
    def get_decoder(self) -> nn.Module:
        """Get the decoder from the language model."""
