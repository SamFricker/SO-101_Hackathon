...
def check_whether_transformers_replace_is_installed_correctly() -> bool:
    ...
    try:
        ...
        # verify the custom siglip patch is present.
        from transformers.models.siglip import modeling_siglip as _siglip_mod

        siglip_encoder_bases = getattr(
            getattr(_siglip_mod, "SiglipEncoderLayer", None), "__bases__", ()
        )
        siglip_base_names = {b.__name__ for b in siglip_encoder_bases}
        if "GradientCheckpointingLayer" not in siglip_base_names:
            return False
