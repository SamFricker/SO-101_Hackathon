...
class MeanStdNormalizer(Normalizer):
    ...
    and actions (target positions). It applies standard z-score normalization:
        ...
        y = (x - mean) / clamp(std, min=1e-6)

    and the corresponding inverse:

        x = y * clamp(std, min=1e-6) + mean
...
class MinMaxNormalizer(Normalizer):
    ...
    range using:
        ...
        y = 2.0 * (x - min) / clamp(max - min, min=1e-6) - 1.0

    and the corresponding inverse:

        x = (y + 1.0) / 2.0 * clamp(max - min, min=1e-6) + min
...
class QuantileNormalizer(Normalizer):
    ...
    It scales inputs to the [-1, 1] range using:
        ...
        y = (x - q01) / clamp(q99 - q01, min=1e-6) * 2.0 - 1.0

    and the corresponding inverse:

        x = (y + 1.0) / 2.0 * clamp(q99 - q01, min=1e-6) + q01
    """
